...
NOTES = [
 "Welcome. Introduce yourself. Set the tone: today is about doing, not listening. One idea (the trust tool), two scales (your desk and a project).",
 "Before we start: everyone scan the three QR codes. Check you can reach the free chat, the companion site, and RetailFlow (code: pilot2024). Do this now so the afternoon doesn't stall.",
 "Quick walk through the day. Morning = personal fluency. Afternoon = project delivery. Same trust tool, two scales.",
 "Read the quote slowly. Hold the silence. 'So where does your edge live?' – this is the question the whole day answers.",
 "One idea (trust tool), two scales (task then project). Taught once, reused all afternoon.",
 "Part one: personal fluency. The judgement that lasts after the next model ships.",
 "AI is bigger than text (ML, vision, audio, CNNs). Today narrows to generative AI. The judgement transfers across the whole field.",
 "This is the visceral proof. NotebookLM, data-to-slides, Teachable Machine – all free, all impressive. The point: 'Everyone in this room can do this by lunchtime. So how do you differentiate?' Don't dwell – 2 min max.",
 "It predicts the next word. Autocomplete, scaled up. Looks like understanding; it's pattern-matching. You don't need the maths.",
 "The ONE mental model. Fluent, plausible, about-right. But 'about right' is not 'exactly right.' Knowing which you need is the whole game.",
 "Tasks where the plausible version IS the answer: drafting, summarising, brainstorming, finding patterns. If a task has patterns, AI can probably help.",
 "Tasks needing one correct answer: facts, figures, reasoning, your context, ethics. It never says 'I don't know.' The fluent-wrong looks identical to the fluent-right.",
 "The dangerous output sounds right. Hallucination isn't a bug – it's the same machinery. Verify, especially facts, numbers, names, citations. This isn't something a future model will fix.",
 "The spine of the day. Two questions: average or precise? small or large? Four corners. Teach it once here; point back to it all afternoon.",
 "Walk through each corner. Average+Small = lean in. Precise+Large = human in the loop. The grid makes 'where does a human stay in the loop?' answerable.",
 "Three examples: email (lean in), board figures (human owns every number), complaints (lean in + skim for the smoothed-away one).",
 "ACTIVITY: 20 min. Pick a real small task, run it through AI, judge with the trust tool. Have a fallback task ready ('chase an overdue invoice').",
 "Detailed exercise instructions. Debrief: share one useful, one confidently-off. Pin each to a grid corner.",
 "Morning tea. 30 min. Encourage people to bring a real task to mind for the RTCF exercise.",
 "Framework slide. Brainstorm, Ideate, Iterate, Amplify, Repeat. Present quickly – 2 min. 'You stay in the conversation.' Depth on the companion site.",
 "Three traps: Gell-Mann amnesia (trust AI less where you know more), AI dismissal fallacy ('just ChatGPT'), sycophancy (it tells you what you want to hear). Awareness is the first defence. 3 min.",
 "ACTIVITY: 25 min. Build RTCF prompts for three real tasks. Emphasize Context = your judgement.",
 "Two halves: scaffolding (R/T/F, needed for weaker models) and Context (always matters – it's your judgement). Don't over-ritualise the labels on a frontier model.",
 "The email example: unusable vs sendable. Context is where your judgement goes in.",
 "ACTIVITY: 15 min. The money moment. One task, run twice: generic then with edge. Let the room shout the edge inputs.",
 "Pass 1: naive prompt, slick, identical for everyone. 'Where's your edge?' Pass 2: add your edge (a customer, a taste call, domain knowledge) – it gets a soul. This is the thesis, felt.",
 "The trust tool says WHEN to check; VET says HOW. Verify (find it independently), Explain (say it in your own words), Test (does it hold up?). The most dangerous output sounds right.",
 "ACTIVITY: 25 min. Redesign one workflow. Map steps, judge each with the trust tool, redesign. One workflow, not the whole job.",
 "AI handles the average steps; you keep the high-stakes. Leave with something to trial next week. Guard against automating everything.",
 "Where we are by lunch. Mental model (convincing average) + grid (trust tool) + method (RTCF). Bridge: 'How do I trust AI across a whole project?'",
 "Proportionate, not prohibitive. Classify data, match the tool, manage real risks (pipeline: transmission/storage/jurisdiction), drop the fictional (model extraction is a myth). Same hygiene as email. 3 min.",
 "ACTIVITY: 10 min. Two-prompt exercise: 'new academic' vs 'new professional staff.' Read side by side. The bias is in the quiet defaults – Dr Chen vs Sarah, window vs no window. The picture does quiet work.",
 "Who's responsible when AI's wrong? You are. Transparency, accountability, HITL. 'The algorithm did it' is not a defence. 3 min.",
 "Lunch. 45 min. Over lunch, think about a real initiative to scope.",
 "Part two: why AI delivery is different. You're the delivery lead.",
 "The five differences aren't LLM-specific. Vision, audio, ML – same failure modes. The judgement travels. 2 min.",
 "'What's actually different about an AI project?' They don't fail more often – they fail differently, in places you didn't see coming.",
 "Walk the table. For each: the assumption, why AI breaks it, the move. Thread: roughly-right, fast – then you decide when good enough and who checks.",
 "Can't specify 'done.' Non-deterministic. Manage a probability distribution. Define 'good enough' upfront for this use.",
 "The demo shows the happy path. Treat it as the START of the hard part, not the end. Sponsors see the demo and assume it's engineering – it isn't.",
 "Data is the uncertainty. Discovered, not ordered. Run delivery as discovery. Build learning into the plan, not as slippage.",
 "Verification IS the product. Design the checking deliberately. Where does a human stay in the loop? A checkpoint is part of what you ship, not an afterthought.",
 "Generic competence is the baseline. When every rival runs the same models, the edge is human variation: taste, judgement, how your people apply it.",
 "A leadership question, not a technical one. It lands on your desk.",
 "You're the delivery lead. Board funded one initiative. Ship it. Interview the bots (code: pilot2024). Treat their output with scrutiny – they're confidently wrong too.",
 "Before you scope: data (do we have it, is it good?), organisation (who owns it, who's affected?), build/buy/host (how do we deliver?). Low readiness reshapes scope.",
 "ACTIVITY: 45 min. Scope against reality. Interview Priya (Data) and Marcus (CIO). Reconcile speed vs data readiness. Don't drift into ROI.",
 "Reconcile 'move fast' (Marcus) with 'data isn't ready' (Priya). Produce scoped objective, data needs, trust-tool split, 'good enough' metric.",
 "Afternoon tea. 30 min. Keep people anchored to their scoped project.",
 "ACTIVITY: 45 min. Stakeholders and HITL. Interview Emma (MD), Tom (frontline), David (CFO). Build stakeholder plan + checkpoint design.",
 "Decide where a human stays in charge. Mark each checkpoint: builds or consumes expertise? Guard against human-everywhere (lose speed) or human-nowhere (lose trust).",
 "ACTIVITY: 45 min. Roadmap, risk, go/no-go. Build it, then defend it. Optional crisis element if time allows. Time-box – protect the 4:00 close.",
 "Roadmap with gates, risk register, then Scale/Pivot/Kill. Make them defend the call with evidence, not just state it.",
 "How to make the go/no-go defensible. Name the 3-5 load-bearing assumptions + sensitivity. Scale if holding, Pivot if shaky-but-fixable, Kill if broken. The register lets you kill with evidence.",
 "The call. Killing early is success. Cost of wrongly scaling is far higher than wrongly killing. Normalise killing – a portfolio that never kills can't learn.",
 "A delivery design: scope, roadmap, checkpoints, risk. Not a theory to apply later. Pause – look at what the trust tool became: a whole project's HITL architecture.",
 "The thesis, restated. The edge is your judgement: taste, experience, critical thinking. Make yours different – the generic version is the one your competitors already have.",
 "Action plan: week/month/quarter. Make commitments specific. Trade plans with a neighbour for accountability. Vague plans don't survive Monday.",
 "Decision tools (trust tool, RTCF, five differences, Scale/Pivot/Kill). Your work. A plan. Go deeper on the companion site – VET, two-chat, voice method, cognitive traps.",
 "Extension: separate thinking (Session 1) from building (Session 2). The curation between is where your judgement lives. Depth on the companion site.",
 "Thank everyone. QR for course feedback. 'Stay the one holding the judgement.' Companion site: michael-borck.github.io/the-human-edge. More: bigpicture.borck.dev.",
]
for _i, _slide in enumerate(prs.slides):
    if _i < len(NOTES):
        _slide.notes_slide.notes_text_frame.text = NOTES[_i]
